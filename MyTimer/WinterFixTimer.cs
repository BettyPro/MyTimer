...
public class WinterFixTimer
{
    ...
    private List<WinterTimeTask> tmpTimeLst = new List<WinterTimeTask>();
    private List<WinterTimeTask> taskTimeLst = new List<WinterTimeTask>();
    private List<int> tmpDelTimeLst = new List<int>();

    private int frameCounter;

    private static readonly string lockFrame = "lockFrame";
    private List<WinterFrameTask> tmpFrameLst = new List<WinterFrameTask>();
    private List<WinterFrameTask> taskFrameLst = new List<WinterFrameTask>();
    private List<int> tmpDelFrameLst = new List<int>();

    public bool isNotUseFrame = false;

    public WinterFixTimer(int interval = 0) {
        tidLst.Clear();
        recTidLst.Clear();
            ...
            lock (lockFrame) {
                for (int i = 0; i < tmpDelFrameLst.Count; i++) {
                    ...
                    for (int j = 0; j < taskFrameLst.Count; j++) {
                        WinterFrameTask task = taskFrameLst[j];
                        if (task.tid == delTid) {
                            isDel = true;
                    ...
                    for (int j = 0; j < tmpFrameLst.Count; j++) {
                        WinterFrameTask task = tmpFrameLst[j];
                        if (task.tid == delTid) {
    ...
    private void CheckFrameTask() {
        ...
        for (int index = 0; index < taskFrameLst.Count; index++) {
            WinterFrameTask task = taskFrameLst[index];
    ...
    public int AddFrameTask(Action<int> callback, int delay, int count = 1) {
        int tid = GetTid();
        lock (lockTime) {
            tmpFrameLst.Add(new WinterFrameTask(tid, callback, frameCounter + delay, delay, count));
        }
        return tid;
    }
    ...
    public bool ReplaceFrameTask(int tid, Action<int> callback, int delay, int count = 1) {
        WinterFrameTask newTask = new WinterFrameTask(tid, callback, frameCounter + delay, delay, count);
    ...
    class WinterTimeTask {
        ...
        public WinterTimeTask(int tid, Action<int> callback, double destTime, double delay, int count) {
            this.tid = tid;
    ...
    class WinterFrameTask {
        ...
        public WinterFrameTask(int tid, Action<int> callback, int destFrame, int delay, int count) {
            this.tid = tid;
            this.callback = callback;
